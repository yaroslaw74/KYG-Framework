...
class InstallController extends AbstractController
{
    ...
    public function requirements(Request $request): Response
    {
        ...
        if ($php) {
            $error = $this->translator->trans('install.error.php', domain: 'install');
        } else {
            $extensions = $this->ArrayAccess->getExtensions();
            $diff = array_diff($extensions['required'], $LoadedExtensions);
            if (count($diff) != 0) {
                $error = $this->translator->trans('install.error.ext', domain: 'install');
            } else {
                if (session_status() == PHP_SESSION_DISABLED) {
                    $error = $this->translator->trans('install.error.session.disabled', domain: 'install');
                } else {
                    if (ini_get('session.auto_start'))
                        $error = $this->translator->trans('install.error.session.autostart', domain: 'install');
                }
            }
        }
